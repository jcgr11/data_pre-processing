...
@st.cache_data
def get_tickers():
    #create a list from the .csv file that contains all tickers
    df = pd.read_csv("C:\py_scripts/mf&etf_tickers.csv") # read .csv file into a pandas dataframe
    tickers = df['ticker'].tolist() #convert the dataframe to a list []
    return tickers
...
def load_data(symbol, start, end):
    ...
    #two different variables were created to make it easier to call for separate streamlit objects 
    data = yf.download(symbol, start_date, end_date, progress=False, auto_adjust=True) #download all data
    price = yf.download(symbol, start_date, end_date, progress=False, auto_adjust=True)['Close'] #download the close price specifically 
